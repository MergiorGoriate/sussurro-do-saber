        ...
        <div className="max-w-[1536px] mx-auto relative z-10">
           <div className="max-w-4xl">
              ...
              <h1 className="text-3xl md:text-6xl font-black mb-5 tracking-tighter leading-none drop-shadow-sm">
                Sussurros do Saber<span className="text-brand-accent">.</span>
              </h1>
              <p className="text-blue-100/80 text-lg md:text-xl font-light mb-10 leading-relaxed max-w-2xl font-serif italic">
                A ponte entre o rigor científico e a literacia cultural. Conhecimento aberto para mentes inquietas.
              </p>
              ...
              <form onSubmit={handleSearchSubmit} className="relative max-w-2xl">
                 <input 
                   ...
                   placeholder="Pesquisar DOI, autor ou conceito..." 
                 ...
                 />
                 <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 p-3 bg-brand-blue rounded-xl text-white hover:bg-slate-900 transition-colors">
                   <Search size={24} />
                 </button>
              </form>
           </div>
        ...
        {tagParam && (
          <div className="mb-8 p-5 bg-white dark:bg-slate-900 border border-brand-blue/20 rounded-[24px] flex items-center justify-between animate-in slide-in-from-top-4 duration-500 shadow-sm">
             <div className="flex items-center gap-4">
                ...
                <div>
                   <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Repositório Filtrado</p>
                   <h3 className="text-lg font-black text-slate-800 dark:text-white">Explorando temas sobre: <span className="text-brand-blue underline decoration-brand-accent/30 decoration-4">#{tagParam}</span></h3>
                </div>
             </div>
             <button onClick={() => navigate('/')} className="p-3 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-slate-400 transition-colors flex items-center gap-2 text-[10px] font-bold uppercase"><X size={18}/> Limpar</button>
          </div>
        )}
          ...
          <aside className="lg:col-span-2 space-y-6 hidden lg:block">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-[24px] shadow-sm sticky top-24">
               <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                 <Filter size={12} /> Áreas de Estudo
               </h3>
               <nav className="space-y-1">
                  {JOURNAL_CATEGORIES.map(cat => (
                    <Link 
                     key={cat.name} 
                     to={`/?cat=${cat.name}`} 
                     className={`group flex items-center justify-between p-2.5 rounded-xl transition-all relative ${
          ...
          <main className="lg:col-span-7 space-y-6">
            <div className="bg-white dark:bg-slate-900 rounded-[24px] border border-slate-200 dark:border-slate-800 p-2 flex items-center justify-between shadow-sm">
               <div className="flex gap-1">
                  <button onClick={() => setFeedTab('recent')} className={`px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${feedTab === 'recent' ? 'bg-brand-blue text-white shadow-lg' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}>Recentes</button>
                  <button onClick={() => setFeedTab('popular')} className={`px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${feedTab === 'popular' ? 'bg-brand-blue text-white shadow-lg' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}>Destaques</button>
               </div>
               <div className="px-4 text-[9px] font-black text-slate-300 dark:text-slate-700 uppercase flex items-center gap-2 tracking-widest">
                 <Database size={12} /> {articles.length} Artigos
               </div>
            </div>

            <div className="space-y-5">
              {isLoading ? (
                <div className="py-20 flex flex-col items-center gap-4 bg-white dark:bg-slate-900 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm">
                  <Loader2 className="animate-spin text-brand-blue" size={32} />
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Acessando Indexador...</span>
                </div>
              ) : articles.length > 0 ? (
                <>
                ...
                <div className="py-28 text-center bg-white dark:bg-slate-900 rounded-[32px] border-2 border-dashed border-slate-200 dark:border-slate-800">
                   ...
                   <h3 className="text-slate-400 text-sm font-black uppercase tracking-widest">Nenhum manuscrito disponível</h3>
                   <Link to="/" className="text-brand-blue text-xs font-black uppercase hover:underline mt-4 inline-block">Limpar Filtros</Link>
                </div>
              )}
            </div>
          </main>
          ...
          <aside className="lg:col-span-3 space-y-6 hidden xl:block">
             <div className="bg-gradient-to-br from-indigo-700 to-brand-blue p-8 rounded-[32px] text-white shadow-2xl relative overflow-hidden group border border-white/10">
                ...
                <div className="mt-8 pt-6 border-t border-white/10 flex justify-between items-center">
                   <span className="text-[9px] font-black uppercase tracking-widest text-blue-300">Fonte: Repositório Sussurros</span>
                   <button onClick={async () => setFact(await apiService.getRandomScienceFact())} className="p-2.5 bg-white/10 hover:bg-white/20 rounded-xl transition-all"><RefreshCw size={14} /></button>
                </div>
             </div>
          </aside>
        </div>
